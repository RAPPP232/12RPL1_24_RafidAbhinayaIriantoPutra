            ...
            <form action="{{ route('parts.update', $part) }}" method="POST">
                ...
                <div class="space-y-4 sm:space-y-6">
                    <div>
                        ...
                        <input type="text" name="code" id="code" value="{{ old('code', $part->code) }}" required
                    ...
                    <div>
                        ...
                        <textarea name="description" id="description" rows="4"
                            class="w-full px-3 sm:px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 text-sm sm:text-base">{{ old('description', $part->description) }}</textarea>
                        @error('description')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
